...
def defectandmirrors2(width: float, length:float, mirrorxlength: float, mirrorlength: float,
    ...
    if findModes:
        ...
        if len(Qs) > 0:
            ...
            with open(os.path.join(dir,"mode_analysis.txt"), "w") as f:
                ...
                f.write(f"  Wavelength: {(c/(freq_maxq*1e12))*1e9:.2f} nm\n")
                if ModeVolume:
                    f.write(f"  Mode volume: {V_mode/freq_max**2:.6f} lambda^2\n")
                    purcell_factor = purcell_prefix * ( wavelength_max_nm/refraction_silicon)**3 * (maxq/ (V_mode * length**2 * thickness) )
    ...
    if modevisulization: 
        #Narrow source to excite the mode you want (change frequency according to what mode you want to excite)
        source_new = mp.Source(mp.GaussianSource(freqs[0], fwidth = .05), excitation, mp.Vector3(0, 0), size = mp.Vector3(0,))
        sim2 = mp.Simulation(
            cell_size = cell, 
            boundary_layers = pml_layers, 
            geometry = geometry, 
...
def DandMirrors3D(width: float, length:float, mirrorxlength: float, mirrorlength: float,
    ...
    sx = 1 + 2 * numholes + beam_padding 
    sy = 2*(pml_padding + padding_to_pml + width)/length  
    sz = 2*(pml_padding + padding_to_pml + thickness)/length  
    cell = mp.Vector3(sx + 2 * pml_padding_x,sy, sz)
    ...
    fcenter = length/wavelength  
    df = .085
    source = mp.Source(mp.GaussianSource(fcenter, fwidth = df), excitation, center = mp.Vector3(0, 0, 0), size = mp.Vector3(0,0,0))
    pml_layers = [mp.PML(pml_padding_x, direction = mp.X),mp.PML(pml_padding/length, direction = mp.Y), mp.PML(pml_padding/length, direction = mp.Z) ]
    if excitation in Magnetic_field:
    ...
    if findModes:
        ...
        if len(Qs) > 0:
            ...
            with open(os.path.join(dir,"mode_analysis.txt"), "w") as f:
                ...
                f.write(f"  Wavelength: {(c/(freq_maxq*1e12))*1e9:.2f} nm\n")
                if ModeVolume:
                    f.write(f"  Mode volume: {V_mode/freq_max**3:.6f} lambda^2\n")
    ...
    if modevisulization: 
        #Narrow source to excite the mode you want (change frequency according to what mode you want to excite)
        source_new = mp.Source(mp.GaussianSource(freqs[0], fwidth = .05), excitation, mp.Vector3(0, 0, 0), size = mp.Vector3(0,0,0))
        sim2 = mp.Simulation(
            cell_size = cell, 
            boundary_layers = pml_layers, 
            geometry = geometry, 
