...
def defectandmirrors(width: float, length:float, xlength: float, ylengths: list, mirrorlength: float, nummirrors: int = 8, excitation = mp.Hz,
                     ...
                     MaxQ_freq: bool = False, resolution: int  = 64, smoothing: bool = True, ModeVolume:bool = True, dir: str =''):
    """
    ...
    if findModes:
        ...
        if len(Qs) > 0:
            ...
            with open(os.path.join(dir,"mode_analysis.txt"), "w") as f:
                ...
                f.write("="*60 + "\n")
                f.write(f"{'Mode':<8} {'Frequency (THz)':<20} {'Q Factor':<15} {'Wavelength (nm)':<15}\n")
                f.write("-"*60 + "\n")
                for i, (freq, q) in enumerate(zip(freqs, Qs)):
                    freq_thz = freq * conv
                    wavelength_nm = (c / (freq_thz * 1e12)) * 1e9
                    f.write(f"{i+1:<8} {freq_thz:<20.6f} {q:<15.2f} {wavelength_nm:<15.2f}\n")
                f.write("\n" + "="*60 + "\n")
                ...
                with open(os.path.join(dir,"mode_data.csv"), "w", newline='') as csvfile:
                    ...
                    writer.writerow(["Mode", "Frequency_THz", "Q_Factor", "Wavelength_nm"])
                    for i, (freq, q) in enumerate(zip(freqs, Qs)):
                        freq_thz = freq * conv
                        wavelength_nm = (c / (freq_thz * 1e12)) * 1e9
                        writer.writerow([i+1, freq_thz, q, wavelength_nm])
